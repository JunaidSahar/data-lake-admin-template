  ...
  <div>
    ...
    <FiltersFilterSideBar>
      <div class="px-5 space-y-6">
        ...
        <FiltersSelectInput
          filter-label="Select payment source"
          @changeValueEmit="
            (value) => {
              (selectedSource = value), getData();
            }
          "
        ...
        <FiltersSelectInput
          filter-label="Select user origin"
          @changeValueEmit="
            (value) => {
              (selectedUserOrigin = value), getData();
            }
          "
...
const sourceOptions = ["none", "easypaisa", "Stripe", "UBL_IPG", "UBL_DD"];
...
const userOriginOptions = ["none", "local", "overseas"];
  ...
  axios
    .get(
      config.public.API_BASE_URL +
        `payments/searchpayment/?page=${currentPage.value}&q=${searchFilterValue.value}&source=${selectedSource.value}&plan=${selectedPlan.value}&origin=${selectedUserOrigin.value}&start_date=${startDate.value}&end_date=${endDate.value}&product=${selectedProduct.value}`,
      {
        headers: {
    ...
    .then((response) => {
      ...
      const responseData = response.data.results.payments;
      allPayments.value = responseData;
